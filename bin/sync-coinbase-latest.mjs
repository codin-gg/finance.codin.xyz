...
const latestCsvWriter = writeCsv(['id', 'base id', 'base name', 'quote id', 'quote name', 'status', 'price', 'volume 24h', 'price % 24h', 'volume % 24h', 'api'])
...
for (const { product_id: id, base_currency_id: baseId, base_name: baseName, quote_currency_id: quoteId, quote_name: quoteName, status, price, volume_24h: volume24h, price_percentage_change_24h: pricePercentageChange24h, volume_percentage_change_24h: volumePercentageChange24h } of products) {
  latestJsonWriter.write({ id: id.toLowerCase(), baseId, baseName, quoteId, quoteName, status, price, volume24h, pricePercentageChange24h, volumePercentageChange24h, api: !['USDC', 'USDT', 'GBP'].includes(quoteId) })
